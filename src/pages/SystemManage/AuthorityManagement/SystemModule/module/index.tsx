...
interface StateType {
  ...
  configSystemModal: LayoutTableModalPropType
}
...
const ModuleConfig = () => {
  ...
  const moduleConfig =
    GlobalConstant.menuPermissionsCode.systemManagement.moduleConfig // 菜单code
  ...
  const handleTableModuleSelect = async (data: AnyObjectType[]) => {
    if (data.length) {
      ...
      uploadModuleSystemData(data[0].id)
    }
  }

  /**
   * @Description 获取系统表格数据
   * @Author bihongbin
  ...
  /**
   * @Description 更新模块下的系统
   * @Author bihongbin
   * @Date 2020-10-28 17:58:02
   */
  const uploadModuleSystemData = useCallback(
    async (id: string) => {
      if (tableSystemRef.current) {
        tableSystemRef.current.setTableLoading(true)
        try {
          const result = await getModuleSystem({
            id,
          })
          setState({
            tableSystemData: result.data,
          })
        } catch (error) {}
        tableSystemRef.current.setTableLoading(false)
      }
    },
    [setState],
  )
  ...
  /**
   * @Description 设置模块下的系统
   * @Author bihongbin
   * @Date 2020-10-28 17:46:37
   */
  const setModuleSystemData = async (data: AnyObjectType[]) => {
    const row = state.tableModuleConfig.row[0]
    try {
      await setModuleSystem({
        id: row.id,
      ...
      uploadModuleSystemData(row.id) // 更新模块下的系统
      ...
      cardSystemHandleButtonList: [
        {
          ...
          clickConfirm: () => {
            setState((prev) => {
              prev.configSystemModal.visible = true
              prev.configSystemModal.title =
                state.tableModuleConfig.row.length &&
                `${state.tableModuleConfig.row[0].moduleCname}-配置系统`
              return prev
            })
          },
        },
  ...
  }, [authBasic.ADD, setState, state.tableModuleConfig.row])
  ...
  useEffect(() => {
    ...
    moduleConfig.code,
    setState,
    state.tableModuleConfig.row,
    uploadModuleSystemData,
  ])
          ...
          render: (value: number, record: any) => {
            ...
            operatingData.push({
              ...
              onClick: () => {
                setState((prev) => {
                  prev.moduleModal.visible = true
                  prev.moduleModal.id = record.id
                  prev.moduleModal.title = '编辑模块'
                  return prev
                })
                getModuleDetails(record.id)
              },
            })
            // 删除
            operatingData.push({
  ...
  }, [authBasic.DELETE, authBasic.EDIT, moduleConfig.code, setState])
   ...
   */
  useEffect(() => {
    if (state.configSystemModal.visible) {
      const tableList = tableSystemRef.current?.getStaticDataList()
      if (tableList) {
        tableModalRef.current?.LayoutTableListRef()?.setRowSelected(
          tableList.reduce<string[]>((arr, item) => {
            arr.push(item.sysId)
            return arr
          }, []),
        )
  ...
  }, [state.configSystemModal.visible])
  ...
  return (
    <div className="mt-4">
      <LayoutTableList
        ref={tableModuleRef}
        api={getModuleList}
        layoutTableListAuthCode={moduleConfig.code}
        ...
        tableColumnsList={{
          ...
          size: state.tableModuleConfig.l,
        ...
        rightRender={{
          size: state.tableModuleConfig.r,
          jsx: (
            <LayoutTableList
              ref={tableSystemRef}
              data={state.tableSystemData}
              searchRightBtnOpen={false}
              cardTopButton={state.cardSystemHandleButtonList}
              cardTopTitle={
                state.tableModuleConfig.row.length &&
                `${state.tableModuleConfig.row[0].moduleCode} ${state.tableModuleConfig.row[0].moduleCname}`
              }
              tableColumnsList={{
      ...
      <LayoutTableModal
        ...
        onCancel={() => {
          setState((prev) => {
            prev.configSystemModal.visible = false
            return prev
          })
        }}
        onConfirm={(data) => setModuleSystemData(data)}
      />
      <LayoutFormModal
        ...
        onConfirm={() => tableModuleRef.current?.getTableList()}
        {...state.moduleModal}
      />
      <IconSelectionModal
